...
getwd()
setwd("C:/Users/shell/Dropbox/Fantastic February")
# load packages
...
#######################################################################################################
# SAR - new code using tidyverse to read data and metadata and subset the columns for analysis
# still relies on static metadata sheet - not synced automatically with raw data
...
view(dataset_columns)
dataset_columns$ID
dataset_final<-filter(dataset_columns,ID != "NA")
dataset_final$ID

dataset_Isla<-filter(dataset_final,Isla_group=="y")
...
colmeta=read_csv("data/metadata/column_metadata.csv")
